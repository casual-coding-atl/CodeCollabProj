...
- **Usernames are app-owned.** Email sign-up requires one; format (3–30, `[a-zA-Z0-9_]`) is enforced by the field's zod `validator.input`, case-insensitive uniqueness by `assertUsernameAvailable` in the user-create hook (`409 { code: 'USERNAME_TAKEN' }`). Better Auth's always-on `POST /update-user` rejects the field entirely (`403 { code: 'USERNAME_CANNOT_BE_UPDATED' }`) — `PUT /api/users/profile` is the only path. New accounts are forced `emailVerified: true` in the same hook (verification is disabled, no sender wired).
  - A member created **through GitHub** never typed one, so the provider's `mapProfileToUser` derives it: `deriveUsername(profile.login, profile.email)` → `sanitizeUsername` (email → local part, non-`[a-zA-Z0-9_]` → `_`, runs collapsed, edges trimmed, padded to 3, truncated to 30, `member` if nothing survives) → case-insensitive collision suffix `alex`, `alex2`, `alex3`, …. It has to be `mapProfileToUser`: `username` is `required: true`, and Better Auth enforces required additional fields on the OAuth create path too, so without it every GitHub sign-up would fail as `?error=username_is_required`.
- **GitHub signs members in _and up_.** `/sign-in/social` is served (it was in `disabledPaths` for one release while the UI was built); the provider has no `disableSignUp`. `authService.signInWithGithub` → `authClient.signIn.social` from the "GitHub" button on `/login` and "Join with GitHub" on `/register`; failures come back as `?error=<code>` and are turned into sentences by `githubSignInFailureMessage`.
  - **`accountLinking.trustedProviders` is deliberately `[]`.** With GitHub untrusted, better-auth links a GitHub identity onto an existing member only when GitHub says the email is **verified** (`!isTrustedProvider && !userInfo.emailVerified` in `oauth2/link-account.mjs`); an unverified one is refused as `?error=account_not_linked`. Naming GitHub as trusted does not *add* that check — it is how you waive it. Don't.
