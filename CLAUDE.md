...
- **Linked Repositories**: `Project.linkedRepos` (at most 3, `{ repoId, owner, name, linkedAt }`), attached and removed on the project edit page via `POST /api/projects/$id/repos` and `DELETE /api/projects/$id/repos/$repoId` (owner-only; the repository must exist on github.com and be public).
